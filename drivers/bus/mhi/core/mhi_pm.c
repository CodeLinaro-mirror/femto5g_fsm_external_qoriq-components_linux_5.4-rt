				...
				enum MHI_PM_STATE state)
{
	...
	if (unlikely(index >= ARRAY_SIZE(mhi_state_transitions))) {
		MHI_CRITICAL(mhi_cntrl, "cur_state:%s is not a valid pm_state\n",
			     to_mhi_pm_state_str(cur_state));
		return cur_state;
	}

	if (unlikely(mhi_state_transitions[index].from_state != cur_state)) {
		MHI_ERR(mhi_cntrl, "index:%u cur_state:%s != actual_state: %s\n",
	...
	MHI_VERB(mhi_cntrl, "Transition to pm state from:%s to:%s\n",
...
int mhi_ready_state_transition(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Waiting to enter READY state\n");
	...
	MHI_LOG(mhi_cntrl, "Device in READY State\n");
	...
	if (cur_state != MHI_PM_POR) {
		MHI_ERR(mhi_cntrl, "Error moving to state %s from %s\n",
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Error programming mmio registers\n");
		goto error_mmio;
	}
...
int mhi_pm_m0_transition(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Entered With State:%s PM_STATE:%s\n",
	...
	if (unlikely(cur_state != MHI_PM_M0)) {
		MHI_ERR(mhi_cntrl, "Failed to transition to state %s from %s\n",
...
void mhi_pm_m1_transition(struct mhi_controller *mhi_cntrl)
{
	...
	if (state == MHI_PM_M2) {
		MHI_VERB(mhi_cntrl, "Entered M2 State\n");
		...
		if (unlikely(atomic_read(&mhi_cntrl->dev_wake))) {
			MHI_VERB(mhi_cntrl, "Exiting M2 Immediately, count:%d\n",
...
int mhi_pm_m3_transition(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Entered mhi_state:%s pm_state:%s\n",
		TO_MHI_STATE_STR(mhi_cntrl->dev_state),
		to_mhi_pm_state_str(mhi_cntrl->pm_state));
	return 0;
}
...
static int mhi_pm_mission_mode_transition(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Processing Mission Mode Transition\n");
...
create_device:
	MHI_LOG(mhi_cntrl, "Adding new devices\n");

	/* add supported devices */
	mhi_create_devices(mhi_cntrl);

	MHI_LOG(mhi_cntrl, "Exited\n");
				      ...
				      enum MHI_PM_STATE transition_state)
{
	...
	MHI_LOG(mhi_cntrl, "Enter with from pm_state:%s MHI_STATE:%s to pm_state:%s\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
		TO_MHI_STATE_STR(mhi_cntrl->dev_state),
	...
	if (cur_state != transition_state) {
		MHI_LOG(mhi_cntrl, "Failed to transition to state:0x%x from:0x%x\n",
	...
	if (MHI_REG_ACCESS_VALID(prev_state)) {
		...
		MHI_LOG(mhi_cntrl, "Trigger device into MHI_RESET\n");
		...
		if ((!ret || in_reset) && cur_state == MHI_PM_SYS_ERR_PROCESS) {
			MHI_CRITICAL(mhi_cntrl, "Device failed to exit RESET state\n");
			mutex_unlock(&mhi_cntrl->pm_mutex);
			return;
		}
	...
	MHI_LOG(mhi_cntrl, "Waiting for all pending event ring processing to complete\n");
	mhi_event = mhi_cntrl->mhi_event;
	for (i = 0; i < mhi_cntrl->total_ev_rings; i++, mhi_event++) {
		if (mhi_event->offload_ev)
			continue;
	...
	MHI_LOG(mhi_cntrl, "Reset all active channels and remove mhi devices\n");
	device_for_each_child(mhi_cntrl->dev, NULL, mhi_destroy_device);

	MHI_LOG(mhi_cntrl, "Finish resetting channels\n");

	/* release lock and wait for all pending thread to complete */
	mutex_unlock(&mhi_cntrl->pm_mutex);
	MHI_LOG(mhi_cntrl, "Waiting for all pending threads to complete\n");
	wake_up_all(&mhi_cntrl->state_event);
	...
	MHI_LOG(mhi_cntrl, "Resetting EV CTXT and CMD CTXT\n");
	...
	if (cur_state == MHI_PM_SYS_ERR_PROCESS) {
		...
		if (unlikely(cur_state != MHI_PM_DISABLE))
			MHI_ERR(mhi_cntrl, "Error moving from pm state:%s to state:%s\n",
				to_mhi_pm_state_str(cur_state),
				to_mhi_pm_state_str(MHI_PM_DISABLE));
	}

	MHI_LOG(mhi_cntrl, "Exit with pm_state:%s mhi_state:%s\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
		TO_MHI_STATE_STR(mhi_cntrl->dev_state));
...
int mhi_debugfs_trigger_reset(void *data, u64 val)
{
	...
	MHI_LOG(mhi_cntrl, "Trigger MHI Reset\n");
	...
	if (!ret || MHI_PM_IN_ERROR_STATE(mhi_cntrl->pm_state)) {
		MHI_ERR(mhi_cntrl, "Did not enter M0 state, cur_state:%s pm_state:%s\n",
			TO_MHI_STATE_STR(mhi_cntrl->dev_state),
			to_mhi_pm_state_str(mhi_cntrl->pm_state));
		return -EIO;
	}
...
void mhi_pm_sys_err_worker(struct work_struct *work)
{
	...
	MHI_LOG(mhi_cntrl, "Enter with pm_state:%s MHI_STATE:%s\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
		TO_MHI_STATE_STR(mhi_cntrl->dev_state));
...
void mhi_pm_st_worker(struct work_struct *work)
{
	...
	list_for_each_entry_safe(itr, tmp, &head, node) {
		list_del(&itr->node);
		MHI_LOG(mhi_cntrl, "Transition to state:%s\n",
...
int mhi_async_power_up(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Requested to power on\n");
	...
	if (!mhi_cntrl->pre_init) {
		...
		if (ret) {
			MHI_ERR(mhi_cntrl, "Error setting dev_context\n");
			goto error_dev_ctxt;
		}

		ret = mhi_init_irq_setup(mhi_cntrl);
		if (ret) {
			MHI_ERR(mhi_cntrl, "Error setting up irq\n");
	...
	if (ret) {
		write_unlock_irq(&mhi_cntrl->pm_lock);
		MHI_ERR(mhi_cntrl, "Error getting bhi offset\n");
	...
	if (mhi_cntrl->fbc_download) {
		...
		if (ret) {
			write_unlock_irq(&mhi_cntrl->pm_lock);
			MHI_ERR(mhi_cntrl, "Error getting bhie offset\n");
			goto error_bhi_offset;
		}
	...
	if (!MHI_IN_PBL(current_ee) && current_ee != MHI_EE_AMSS) {
		MHI_ERR(mhi_cntrl, "Not a valid ee for power on\n");
		ret = -EIO;
		goto error_bhi_offset;
	}
	...
	MHI_LOG(mhi_cntrl, "Power on setup success\n");
...
void mhi_power_down(struct mhi_controller *mhi_cntrl, bool graceful)
{
	...
	if (!graceful) {
		...
		if (cur_state != MHI_PM_LD_ERR_FATAL_DETECT)
			MHI_ERR(mhi_cntrl, "Failed to move to state:%s from:%s\n",
...
int mhi_pm_suspend(struct mhi_controller *mhi_cntrl)
{
	...
	if (atomic_read(&mhi_cntrl->dev_wake)) {
		MHI_VERB(mhi_cntrl, "Busy, aborting M3\n");
		return -EBUSY;
	}
	...
	MHI_LOG(mhi_cntrl, "Allowing M3 transition\n");
	new_state = mhi_tryset_pm_state(mhi_cntrl, MHI_PM_M3_ENTER);
	if (new_state != MHI_PM_M3_ENTER) {
		write_unlock_irq(&mhi_cntrl->pm_lock);
		MHI_ERR(mhi_cntrl, "Error setting to pm_state:%s from pm_state:%s\n",
	...
	write_unlock_irq(&mhi_cntrl->pm_lock);
	MHI_LOG(mhi_cntrl, "Wait for M3 completion\n");
	...
	if (!ret || MHI_PM_IN_ERROR_STATE(mhi_cntrl->pm_state)) {
		MHI_ERR(mhi_cntrl, "Did not enter M3 state, cur_state:%s pm_state:%s\n",
			TO_MHI_STATE_STR(mhi_cntrl->dev_state),
			to_mhi_pm_state_str(mhi_cntrl->pm_state));
		return -EIO;
	}
...
int mhi_pm_resume(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Entered with pm_state:%s dev_state:%s\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
		TO_MHI_STATE_STR(mhi_cntrl->dev_state));
...
int mhi_force_rddm_mode(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Enter with pm_state:%s ee:%s\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
	...
	MHI_LOG(mhi_cntrl, "Triggering SYS_ERR to force rddm state\n");
	...
	/* wait for rddm event */
	MHI_LOG(mhi_cntrl, "Waiting for device to enter RDDM state\n");
	ret = wait_event_timeout(mhi_cntrl->state_event,
				 mhi_cntrl->ee == MHI_EE_RDDM,
				 msecs_to_jiffies(mhi_cntrl->timeout_ms));
	ret = !ret ? 0 : -EIO;

	MHI_LOG(mhi_cntrl, "Exiting with pm_state:%s ee:%s ret:%d\n",
		to_mhi_pm_state_str(mhi_cntrl->pm_state),
