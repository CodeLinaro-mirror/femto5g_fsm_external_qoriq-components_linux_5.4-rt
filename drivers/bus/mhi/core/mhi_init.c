...
int mhi_init_irq_setup(struct mhi_controller *mhi_cntrl)
{
	...
	for (i = 0; i < mhi_cntrl->total_ev_rings; i++, mhi_event++) {
		...
		ret = request_irq(mhi_cntrl->irq[mhi_event->msi],
				  ...
				  mhi_event);
		if (ret) {
			MHI_ERR(mhi_cntrl, "Error requesting irq:%d for ev:%d\n",
...
int mhi_init_timesync(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Failed to send time sync cfg cmd\n");
		goto error_send_cmd;
	}
	...
	if (!ret || mhi_tsync->ccs != MHI_EV_CC_SUCCESS) {
		MHI_ERR(mhi_cntrl, "Failed to receive cmd completion for time_sync_cfg\n");
		ret = -EIO;
		goto error_send_cmd;
	}
	...
	ret = mhi_get_capability_offset(mhi_cntrl, TIMESYNC_CAP_ID,
					&time_offset);
	if (ret) {
		MHI_ERR(mhi_cntrl, "could not find timesync capability\n");
		goto error_sync_cap;
	}
	...
	MHI_LOG(mhi_cntrl, "TIMESYNC_DB OFFS:0x%x\n", db_offset);
...
int mhi_init_mmio(struct mhi_controller *mhi_cntrl)
{
	...
	MHI_LOG(mhi_cntrl, "Initializing MMIO\n");
	...
	MHI_LOG(mhi_cntrl, "CHDBOFF:0x%x\n", val);
	...
	MHI_LOG(mhi_cntrl, "ERDBOFF:0x%x\n", val);
	...
	MHI_LOG(mhi_cntrl, "Programming all MMIO values.\n");
...
int mhi_prepare_for_power_up(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Error with init dev_ctxt\n");
		goto error_dev_ctxt;
	}
...
static int mhi_driver_remove(struct device *dev)
{
	...
	MHI_LOG(mhi_cntrl, "Removing device for chan:%s\n", mhi_dev->chan_name);
