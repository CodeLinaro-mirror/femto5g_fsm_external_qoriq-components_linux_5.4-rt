...
int mhi_arch_link_suspend(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Failed with pci_save_state, ret:%d\n", ret);
		goto exit_suspend;
	}
...
exit_suspend:
	MHI_LOG(mhi_cntrl, "Exited with ret:%d\n", ret);
...
int mhi_arch_link_resume(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Failed to set PCI_D0 state, ret:%d\n", ret);
		return ret;
	}

	ret = pci_enable_device(pci_dev);
	if (ret) {
		MHI_ERR(mhi_cntrl, "Failed to enable device, ret:%d\n", ret);
		return ret;
	}

	ret = pci_load_and_free_saved_state(pci_dev, &arch_info->pcie_saved_state);
	if (ret)
		MHI_LOG(mhi_cntrl, "Failed to load saved cfg state\n");
...
static int mhi_init_pci_dev(struct mhi_controller *mhi_cntrl)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Error assign pci resources, ret:%d\n", ret);
		return ret;
	}

	ret = pci_enable_device(pci_dev);
	if (ret) {
		MHI_ERR(mhi_cntrl, "Error enabling device, ret:%d\n", ret);
		goto error_enable_device;
	}

	ret = pci_request_region(pci_dev, mhi_dev->resn, "mhi");
	if (ret) {
		MHI_ERR(mhi_cntrl, "Error pci_request_region, ret:%d\n", ret);
	...
	if (!mhi_cntrl->regs) {
		MHI_ERR(mhi_cntrl, "Error ioremap region\n");
		goto error_ioremap;
	}
	...
	if (IS_ERR_VALUE((ulong)ret) || ret < mhi_cntrl->msi_required) {
		MHI_ERR(mhi_cntrl, "Failed to enable MSI, ret:%d\n", ret);
		goto error_req_msi;
	}
...
static int mhi_runtime_suspend(struct device *dev)
{
	...
	MHI_LOG(mhi_cntrl, "Enter\n");

	mutex_lock(&mhi_cntrl->pm_mutex);

	if (!mhi_dev->powered_on) {
		MHI_LOG(mhi_cntrl, "Not fully powered, return success\n");
	...
	if (ret) {
		MHI_LOG(mhi_cntrl, "Abort due to ret:%d\n", ret);
		goto exit_runtime_suspend;
	}

	ret = mhi_arch_link_off(mhi_cntrl, true);
	if (ret)
		MHI_ERR(mhi_cntrl, "Failed to Turn off link ret:%d\n", ret);
...
{
	struct mhi_controller *mhi_cntrl = dev_get_drvdata(dev);
	MHI_LOG(mhi_cntrl, "Entered returning -EBUSY\n");
...
rpm_resume_exit:
	mutex_unlock(&mhi_cntrl->pm_mutex);
	MHI_LOG(mhi_cntrl, "Exited with :%d\n", ret);
...
static int mhi_system_resume(struct device *dev)
{
	...
	if (ret) {
		MHI_ERR(mhi_cntrl, "Failed to resume link\n");
	} else {
			  ...
			  enum MHI_CB reason)
{
	...
	if (reason == MHI_CB_IDLE) {
		MHI_LOG(mhi_cntrl, "Schedule runtime suspend 1\n");
...
int mhi_debugfs_trigger_m0(void *data, u64 val)
{
	...
	MHI_LOG(mhi_cntrl, "Trigger M3 Exit\n");
...
int mhi_debugfs_trigger_m3(void *data, u64 val)
{
	...
	MHI_LOG(mhi_cntrl, "Trigger M3 Entry\n");
                        ...
                        const struct mhi_device_id *id)
{
	...
	MHI_LOG(mhi_cntrl, "Probing Successful\n");
	return 0;
}
...
int mhi_bl_init(void)
{
        return mhi_driver_register(&mhi_bl_driver);
}
		  ...
		  const struct pci_device_id *device_id)
{
	...
	MHI_LOG(mhi_cntrl, "Return successful\n");
