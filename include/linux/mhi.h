...
#define MHI_VERB(mhi_cntrl, fmt, ...)  do { \
	if ((mhi_cntrl != NULL) && (mhi_cntrl->klog_lvl < MHI_MSG_LVL_VERBOSE)) \
		pr_info("[D] %04x:%02x:%02x [%s] " fmt, \
			mhi_cntrl->domain, mhi_cntrl->bus, \
			mhi_cntrl->slot, __func__, ##__VA_ARGS__);\
} while (0)

#else

#define MHI_VERB(mhi_cntrl, fmt, ...)

#endif

#define MHI_LOG(mhi_cntrl, fmt, ...)  do { \
	if (mhi_cntrl != NULL) \
		pr_info("[I] %04x:%02x:%02x [%s] " fmt, \
			mhi_cntrl->domain, mhi_cntrl->bus, \
			mhi_cntrl->slot, __func__, ##__VA_ARGS__);\
	else \
...
#define MHI_ERR(mhi_cntrl, fmt, ...)  do { \
	if (mhi_cntrl != NULL) \
		pr_info("[E] %04x:%02x:%02x [%s] " fmt, \
			mhi_cntrl->domain, mhi_cntrl->bus, \
			mhi_cntrl->slot, __func__, ##__VA_ARGS__);\
	else \
		pr_info("[E][%s] " fmt, __func__, ##__VA_ARGS__);\
} while (0)

#define MHI_CRITICAL(mhi_cntrl, fmt, ...)  do { \
	if (mhi_cntrl != NULL) \
		pr_info("[C] %04x:%02x:%02x [%s] " fmt, \
			mhi_cntrl->domain, mhi_cntrl->bus, \
			mhi_cntrl->slot, __func__, ##__VA_ARGS__);\
	else \
		pr_info("[C][%s] " fmt, __func__, ##__VA_ARGS__);\
} while (0)
